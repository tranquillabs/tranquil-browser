  ...
  return {
    ...
    'print': (b) => {
      try {
        ...
        console.error('[tranquil-browser] print failed:', e);
      }
    },
  };
}
...
// Map a keydown (a real KeyboardEvent, or the plain w_event forwarded from a
// guest webview) to a routable command id (sans `tranquil-browser:` prefix), or
// null. One place for the case/modifier normalization the guest keyHandler and
// the keymap must agree on. The cmd-k split chord is handled separately (split-
// chord.js); F12/F5/F10 have no command and stay view-local.
function keyToCommandId(evt) {
  const isMac = process.platform === 'darwin';
  const mod = isMac ? evt.metaKey : evt.ctrlKey; // primary (cmd on mac, ctrl else)
  const shift = !!evt.shiftKey;
  const key = String(evt.key || '').toLowerCase();

  // alt combos (no primary modifier): alt-d focuses the URL bar. On Windows/Linux,
  // alt-arrows are the platform-standard history chord (back/forward); on macOS
  // that chord is cmd-arrows (handled below), so alt-arrows stay view-local there.
  if (evt.altKey && !mod) {
    if (key === 'd') return 'focus-url';
    if (!isMac) {
  ...
  // cmd-arrows are the platform-standard history chord on macOS (back/forward).
  if (isMac && !shift) {
    if (key === 'arrowleft') return 'go-back';
    if (key === 'arrowright') return 'go-forward';
  }
  ...
  switch (key) {
    ...
    case 'r':
      return shift ? 'hard-refresh' : 'refresh';
    case 't':
      return shift ? null : 'open';
    case 'l':
      return 'focus-url';
    case 'a':
      return shift ? 'add-to-tree-view' : null;
    case 'p':
      return shift ? null : 'print';
    case 's':
      return shift ? null : 'save-url';
    default:
      return null;
  }
}

module.exports = { activeBrowser, buildCommands, keyToCommandId };
