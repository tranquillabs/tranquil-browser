...
const registerGlobalIpcEvents = (atom, TranquilBrowser) => {
  ...
  ipcRenderer.on('open-link-in-new-tab', (event, args) => {
    ...
    if (linkToOpen) {
      // Snapshot the pane before opening: workspace.open dedupes by URI, so a
      // link whose URL matches an already-open tab resolves to that EXISTING
      // item. Those must not be stack-placed — moveItem would yank an
      // established tab out of position. This bit hardest on session restore:
      // every restored webview re-runs its page scripts, and any programmatic
      // window.open (SSO redirects, ads) whose URL matched an open tab
      // reshuffled the restored tab order on every launch.
      const preexisting = new Set(activePane.getItems());
      atom.workspace
        .open(linkToOpen, { activateItem: foreground, activatePane: foreground })
        .then((newItem) => {
          if (preexisting.has(newItem)) return;
...
// Chrome — with the Chrome version token swapped for a current release. The
// bundled Chromium's own version freezes with each Electron release and ages
// into a bot signal (HN 429s /login for Chrome/124). The current version comes
// from `tranquil.browserVersions` (the "Check Current Versions" action in the
// Tranquil settings tab), with a pinned fallback for before the first check —
// keep the fallback in sync with UA_VERSION_FALLBACKS in
// tranquil-config/src/index.js. A function (not a const) so a version check
// applies without a reload.
const CLEAN_UA_CHROME_FALLBACK = "139.0.0.0";
const cleanUserAgent = () => {
  const stored = atom.config.get("tranquil.browserVersions") || {};
  return navigator.userAgent
    .replace(/ (?:Tranquil|Electron)\/\S+/g, "")
    .replace(/Chrome\/[\d.]+/, `Chrome/${stored.chrome || CLEAN_UA_CHROME_FALLBACK}`);
};
...
const resolveUserAgent = () => {
  ...
  return override || cleanUserAgent();
};
