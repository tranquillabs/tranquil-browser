...
const addIpcInstanceEvents = (webViewItem, atom, TranquilBrowser) => {
  ...
  ipcRenderer.on('open-link-in-new-tab', (event, args) => {
    ...
    const historyPane = atom.workspace.getActivePane();
    const linkToOpen = args?.link ? args.link : args?.text;
    if (linkToOpen) {
      if (args.foreground) {
        const otherPane = atom.workspace.getPanes().find(p => p !== historyPane);
        if (otherPane) {
          otherPane.activate();
          atom.workspace.open(linkToOpen);
        } else {
          atom.workspace.open(linkToOpen, { split: 'right' });
        }
      } else {
        atom.workspace.open(linkToOpen).then(() => {
          historyPane.activate();
          historyPane.activatePreviousItem();
        });
      }
    }
  });
