...
const TranquilBrowserView = (function (superClass) {
  ...
  TranquilBrowserView.prototype.keyHandler = function (evt) {
    ...
    switch (evt.key) {
      ...
      case "s":
      case "S":
        // cmd/ctrl+s → save this tab as a .url file. Handled here because a
        // focused guest webview swallows the keystroke before Atom's keymap.
        // Exclude shift so cmd-shift-s keeps its own binding.
        if ((isMac ? evt.metaKey : evt.ctrlKey) && !evt.shiftKey) {
          return TranquilBrowser.saveCurrentTabUrl();
        }
        break;
      case "d":
        if (evt.altKey) {
