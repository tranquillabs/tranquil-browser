...
const TranquilBrowserView = (function (superClass) {
  ...
  TranquilBrowserView.prototype.initialize = function () {
    ...
    // These use kebab-case ids (go-back, not goBack) so they humanize to
    // multi-word names. tranquil-config relabels the whole tranquil-browser
    // namespace to "Browser: …" in the command palette.
    this.subscriptions.add(
      atom.commands.add(".tranquil-browser webview", {
        "tranquil-browser:go-back": (function (_this) {
          return function () {
            return _this.goBack();
          };
        })(this),
    ...
    this.subscriptions.add(
      atom.commands.add(".tranquil-browser webview", {
        "tranquil-browser:go-forward": (function (_this) {
          return function () {
    ...
    this.subscriptions.add(
      atom.commands.add(".tranquil-browser", {
        "tranquil-browser:toggle-url-bar": (function (_this) {
          return function () {
  ...
  TranquilBrowserView.prototype.keyHandler = function (evt) {
    ...
    // opt+cmd+w (Ctrl+Alt+W on Win/Linux): close this tab and trash its .url.
    // Match on evt.code, not evt.key — macOS mangles evt.key to a dead char
    // (e.g. "∑") while Option is held.
    if ((isMac ? evt.metaKey : evt.ctrlKey) && evt.altKey && evt.code === 'KeyW') {
      TranquilBrowser.closeTabAndTrashUrl(this.model);
      return;
    }
