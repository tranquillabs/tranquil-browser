...
const fs = require("fs");
require("jstorage");
const { addIpcInstanceEvents, addUrlChangeInstanceEvent, windowHistoryKey, tabDisplayTitle, resolveUserAgent } = require("./utils");
const TranquilBrowser = require("./tranquil-browser");
const { notify } = require("./notify.js");
const SplitChord = require("./split-chord");
const { keyToCommandId } = require("./browser-commands");
const suggestions = require("./history-suggestions");

const View = ref.View;
const $ = ref.$;

// The guest's URL, or '' when it cannot be read yet.
//
// webview.getURL() THROWS ("The WebView must be attached to the DOM and the dom-ready event
// emitted before this method can be called") when it is called before the guest is attached.
// That happens on window reload: a restored browser tab emits page-title-set and
// page-favicon-updated while its <webview> is still being attached, and the handlers below read
// the URL to key their bookmark/title caches.
//
// The throw is uncaught, and Pulsar responds to any uncaught renderer error by opening DevTools
// and switching it to the console (AtomEnvironment.installUncaughtErrorHandler) — so a harmless
// early event popped the debugger open on every reload of a window with a browser tab in it.
//
// The call sites already guard with `if (!uri) return;`, written as though getURL() returned
// falsy when unavailable. This makes that guard true.
function guestUrl(webview) {
  try {
...
const TranquilBrowserView = (function (superClass) {
  ...
  function TranquilBrowserView(model) {
    ...
    this.model.onDidDestroy(() => {
      ...
      // Closing the tab detaches the <webview> next, which normally tears the
      // guest down almost instantly. A page stuck in heavy/blocking JS can
      // leave that renderer unresponsive to the teardown, so cmd-w just
      // hangs. Grab the guest's webContents id now, while the element
      // reference is still live, and give it a grace period — if it's still
      // around after that, the main process (cz-init.js, 'force-close-webview')
      // kills the renderer outright so the tab actually closes.
      ...
      if (webContentsId) {
        setTimeout(() => {
          ipcRenderer.send("force-close-webview", webContentsId);
        }, 750);
      }
    });
  ...
  TranquilBrowserView.content = function (params) {
    ...
    this.div({ class: "tranquil-browser" }, () => {
      ...
      const webviewAttrs = {
        ...
        plugins: "on",
        src: `${url}`,
        // Present a clean, stock-browser User-Agent to the guest (Electron/Tranquil
        // tokens stripped; configurable via tranquil.browserUserAgent). The attribute
        // applies before first navigation and persists across in-tab navigations, so
        // it covers this tab without a main-process round-trip.
      ...
      // Browser-style hover-link preview: a pill anchored bottom-left over the
      // webview, shown only while the cursor is over a link (fed by the guest's
      // native `update-target-url` event). Hidden by default.
      this.div({ class: "hover-url-bc", outlet: "hoverUrl" });
    });
  };
  ...
  // Show/hide the browser-style hover-link pill. Called with the hovered link's
  // href (from the guest's native `update-target-url` event) or an empty string
  // to hide it.
  TranquilBrowserView.prototype.showHoverUrl = function (url) {
    const el = this.hoverUrl && this.hoverUrl[0];
    if (!el) return;
    if (url) {
      el.textContent = url;
  ...
  // Focus + select the URL bar input. Shared by the focus-url command (cmd-l) and
  // the guest keyHandler (cmd-l / alt-d) so the "select the address" behaviour
  // lives in one place.
  TranquilBrowserView.prototype.focusUrlBar = function () {
    const input = this.urlbar?.[0]?.querySelector("#url");
    if (input) {
      input.focus();
      input.select();
    }
  ...
  // Turn a search-suggestion string into where selecting it should go: a bare
  // domain / URL navigates straight there; anything else is a DuckDuckGo search.
  // Mirrors the URL bar's own Enter handling and the start page's targetFor, so a
  // suggestion like "news.ycombinator.com" opens the site instead of searching
  // for it, while "news ycombinator" (has a space) still searches.
  TranquilBrowserView.prototype.suggestionTarget = function (text) {
    var v = (text || "").trim();
    if (!v) return "";
    if (/^[a-z][a-z0-9+.-]*:\/\//i.test(v)) return v; // already a full URL
    if (
      v.indexOf(" ") < 0 &&
      (v.indexOf(".") >= 0 || /^localhost(:\d+)?(\/|$)/i.test(v))
    ) {
      return "https://" + v;
    }
  ...
  // Deliberate navigations (Enter in the URL bar, suggestion pick) feed the
  // typed-URL boost that ranks those pages above merely-visited ones in the
  // dropdown. HAR-replay tabs are excluded — replaying an archive isn't
  // browsing intent. Search values no-op inside recordTypedUrl, so callers
  // don't need to distinguish navigate-vs-search first.
  TranquilBrowserView.prototype.recordTypedNavigation = function (url) {
    if (this.model && this.model.opt && this.model.opt.isHarReplay) return;
    suggestions.recordTypedUrl(url);
  };
  ...
  TranquilBrowserView.prototype.initialize = function () {
    var base1,
      ...
      select,
      src;
    // Standard focus() contract on the view's root element: focus the guest
    // <webview>. atom.views.getView(model) caches and returns this same node
    // (ViewRegistry, getViewClass branch), so core's pane-element delegation
    // (activeItemChanged / handleFocus → getActiveView().focus()) focuses the
    // guest through Atom's own machinery — no onDidChangeActivePaneItem side-channel.
    const self = this;
    this[0].focus = function () {
      const wv = self.htmlv && self.htmlv[0];
      if (wv) {
        wv.focus();
      } else {
    ...
    // Suggestion source, two-phase (verified against the bundled jquery-ui
    // 1.10.5: each res() call re-renders while this is still the newest
    // request, and the widget's requestIndex guard drops replies for
    // superseded terms). Phase 1 is synchronous — frecency-ranked history
    // aggregated across every window, then matching favorites. Phase 2
    // re-responds with DuckDuckGo search suggestions appended (Node https;
    // replaces the old plain-http Bing endpoint, which also failed silently).
    // Every item must be a full {label, value} object: jquery-ui's _normalize
    // short-circuits on the first item's shape, and history objects lead the
    // array.
    src = (function (_this) {
      return function (req, res) {
        var view = _this;
        var items = suggestions.localSuggestions(req.term, 6, 10);
        res(items);
        suggestions.fetchSearchSuggestions(req.term).then(function (list) {
          res(
            suggestions.appendSearchSuggestions(
              items,
              list,
              function (s) {
                // A bare domain/URL suggestion opens the site; everything else
                // searches (also dedupes against phase-1 rows by destination).
                return view.suggestionTarget(s);
              },
              10
            )
          );
    ...
    select = (function (_this) {
      return function (event, ui) {
        _this.recordTypedNavigation(ui.item.value);
    ...
    if (typeof (base1 = jQ(this.url)).autocomplete === "function") {
      ...
      // Per-row renderer (icon + title + dimmed URL). jquery-ui 1.10 has no
      // .autocomplete("instance"); the widget hangs off jQuery data — and it
      // must be read through jQ, the widget's own jQuery, not space-pen's $
      // (separate data expandos).
      const widget = base1.data("ui-autocomplete");
      if (widget) {
        widget._renderItem = function (ul, item) {
          return suggestions.renderItem(jQ, ul, item);
        };
      }
      // Start each focus session with a fresh cross-window history index
      // (other windows' visits arrive via jStorage's storage observer).
    ...
    if ((ref5 = this.htmlv[0]) != null) {
      ref5.addEventListener(
        ...
        (function (_this) {
          return function (evt) {
            // Clear a stale hover pill so it never survives a page change.
            _this.showHoverUrl("");
    ...
    // Browser-style hover-link preview: Chromium fires this native webview event
    // with the link's href on mouse-over and an empty string on mouse-out, so the
    // pill self-clears — no guest-side mouse forwarding required.
    if (this.htmlv[0] != null) {
      this.htmlv[0].addEventListener(
        "update-target-url",
        (function (_this) {
          return function (evt) {
            return _this.showHoverUrl(evt.url);
          };
        })(this)
      );
    }
    if (this.htmlv[0] != null) {
      this.htmlv[0].addEventListener(
        ...
        (function (_this) {
          return function (e) {
            // A guessed bare-domain navigation (from the URL bar or a search
            // suggestion) whose host doesn't resolve otherwise leaves a blank
            // white page. Fall back to searching for that host so e.g. a
            // suggestion like "news.ycombinator" (no real TLD) still does
            // something useful. Scoped to the main frame and name-resolution
            // failures only — a real host that's merely down (connection
            // refused/timeout) keeps its normal error page — and never re-fires
            // on the search URL itself (no loop).
            //   -105 ERR_NAME_NOT_RESOLVED, -137 ERR_NAME_RESOLUTION_FAILED
            if (!e.isMainFrame) return;
            if (e.errorCode !== -105 && e.errorCode !== -137) return;
            var failed = e.validatedURL || "";
            if (/^https?:\/\/(www\.)?duckduckgo\.com\//i.test(failed)) return;
            var term;
            try {
              var u = new URL(failed);
              term = (u.hostname + u.pathname).replace(/\/+$/, "");
            } catch (err) {
              return;
            }
            if (!term) return;
    ...
    // Links that open a new tab (target="_blank" / window.open) are handled in
    // the guest preload (resources/bp-client.js, which routes them through the
    // open-link-in-new-tab flow) — NOT here. Electron 30's <webview> no longer
    // emits the `new-window` event, so a handler on it would never fire.
    ...
    this.url.on("keydown", function (evt) {
      // The address bar swallows keydowns so app/editor keymaps don't fire while
      // typing a URL. But global tab-cycling must still work from here — let
      // Ctrl+Tab / Ctrl+Shift+Tab (keyCode 9) reach Atom's keymap. (cmd-w etc.
      // arrive via Electron menu accelerators, which bypass this handler.)
      ...
      (function (_this) {
        return function (evt) {
          ...
          if (evt.which === 13) {
            ...
            _this.recordTypedNavigation(url);
        ...
        if (url) {
          ...
          if (ignorecache) {
            ...
            try {
              ...
              console.error('[tranquil-browser] Webview reload failed:', e);
            }
          }
        }
      }
    } catch (e) {
      console.error('[tranquil-browser] refreshPage failed:', e);
    }
  };
  ...
  TranquilBrowserView.prototype.keyHandler = function (evt) {
    ...
    // cmd-k / ctrl-k + arrow: split chord (single shared owner in split-chord.js).
    // keyHandler runs only when this browser is the active item, so split its pane.
    if (SplitChord.handle(evt, () => atom.workspace.paneForItem(this.model))) {
      return;
    }

    // View-local keys that have no workspace command.
    switch (evt.key) {
      case "F12":
        return this.toggleDevTool();
      case "F5":
        return this.refreshPage(void 0, evt.ctrlKey || (isMac && evt.metaKey));
      case "F10":
        return this.toggleURLBar();
    }

    // Everything else is a routable browser shortcut. A focused guest webview
    // swallowed the key before Atom's keymap could see it, so dispatch the same
    // command the keymap uses — one source of truth in browser-commands.js.
    // keyHandler only fires when this browser is the active item, so the
    // command's activeBrowser() resolves back to it. (Ctrl+Tab tab-cycling is
    // intercepted earlier in the webview-key-events handler, before the
    // active-item guard, so it never reaches keyHandler.)
    const id = keyToCommandId(evt);
    if (id) {
  ...
  TranquilBrowserView.prototype.addHistory = function () {
    ...
    // HAR-replay tabs replay an archived session — their navigations are not
    // user browsing and shouldn't enter history (or, via it, suggestions).
    if (this.model.opt && this.model.opt.isHarReplay) {
      return;
    }
    url = guestUrl(this.htmlv[0]).replace(/\\/g, "/");
    if (!url) {
      return;
    }
