...
const TranquilBrowser = {
  ...
  // Targets the CENTER's active item (not the focused pane), so it works no
  // matter which dock has focus. Returns false when the active center item isn't
  // a browser, so the caller can fall through to core:save and editors keep their
  // normal cmd-s.
  saveCurrentTabUrl() {
  ...
  activate: function (state) {
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        // cmd-l selects the URL bar of the active browser tab. Bound at the
        // workspace level so it fires from anywhere within a browser tab (pane
        // focus, url bar, etc.); the guest-webview-focused case is handled
        // separately in the view's keyHandler. For non-browser items, abort so
        // the keystroke falls through to the editor's normal cmd-l (select-line).
        'tranquil-browser:focus-url': function (event) {
          const item = atom.workspace.getCenter().getActivePaneItem();
          if (item instanceof TranquilBrowserModel) {
            const urlInput = item.view?.urlbar[0]?.querySelector('#url');
            urlInput?.focus();
            urlInput?.select();
        ...
        // cmd-s / ctrl-s saves the active browser tab as a `.url` file. Bound at
        // the workspace level so it fires no matter which dock has focus; the
        // guest-webview-focused case is handled separately in the view's
        // keyHandler. For non-browser items, abort so the keystroke falls through
        // to the editor's normal cmd-s (core:save).
        'tranquil-browser:save-url': function (event) {
          if (
            !TranquilBrowser.saveCurrentTabUrl() &&
            event &&
            typeof event.abortKeyBinding === 'function'
          ) {
            event.abortKeyBinding();
          }
        },
