...
const { windowHistoryKey } = require("./utils");
require("JSON2");
...
const TranquilBrowser = {
  ...
  // url -> { filePath, content } for a bookmark .url trashed by
  // close-tab-and-delete-url. reopen-closed-item (shift-cmd-t) reopens BY URL
  // (see TranquilBrowserModel#getURI), so we restore the file at its original
  // path and re-bookmark the reopened tab when that URL comes back.
  _trashedUrlFiles: new Map(),
  addToTreeView(url) {
  ...
  closeTabAndTrashUrl(item) {
    ...
    if (filePath && filePath.endsWith('.url') && fs.existsSync(filePath)) {
      // Remember the content (keyed by URL, how reopen tracks it) before trashing
      // so reopen-closed-item can restore the file at its original path.
      let content = null;
      try { content = fs.readFileSync(filePath, 'utf8'); } catch (e) {}
      const url = (item.getURL() || '').replace(/\/$/, '');
      require('electron').shell.trashItem(filePath).then(
        () => {
          if (content != null && url) {
            TranquilBrowser._trashedUrlFiles.set(url, { filePath, content });
          }
  ...
  activate: function (state) {
    ...
    // Restore a bookmark tab reopened via shift-cmd-t after close-tab-and-delete-url
    // trashed its .url. reopen-closed-item reopens by URL, so the tab comes back as
    // a plain browser tab; when its URL matches a trashed entry, rewrite the .url
    // file at its original path and re-bookmark the tab (set filePath) so the tree
    // view can follow it again.
    this.subscriptions.add(
      atom.workspace.onDidAddPaneItem(({ item }) => {
        if (!(item instanceof TranquilBrowserModel)) return;
        const url = (item.getURL() || '').replace(/\/$/, '');
        const trashed = url && TranquilBrowser._trashedUrlFiles.get(url);
        if (!trashed) return;
        TranquilBrowser._trashedUrlFiles.delete(url);
        try {
          if (!fs.existsSync(trashed.filePath)) {
            fs.writeFileSync(trashed.filePath, trashed.content);
          }
          item.filePath = trashed.filePath;
          // The .url was just recreated on disk. The tree-view renders it via its
          // fs watcher (async) and won't re-reveal the already-active tab on its
          // own, so nudge reveal-active-file a few times until the restored entry
          // has rendered and gets selected. entryForPath returns the nearest
          // ancestor until then, so reveal only expands the parent and never
          // throws; once the entry exists a later nudge selects it.
          const wsEl = atom.views.getView(atom.workspace);
          [100, 250, 450, 700, 1000].forEach((ms) =>
            setTimeout(() => {
              if (atom.workspace.getCenter().getActivePaneItem() === item) {
                atom.commands.dispatch(wsEl, 'tree-view:reveal-active-file');
              }
            }, ms)
          );
        } catch (e) {
          // Couldn't restore the file — leave the reopened tab as a plain tab.
        }
      })
    );
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        ...
        // cmd-f/ctrl-f is bound at the workspace level so it wins over
        // find-and-replace's less-specific `.platform-darwin` binding no matter
        // where focus sits in a browser tab. For non-browser items, abort so the
        // keystroke falls through to find-and-replace's normal editor find.
        'tranquil-browser:find': function (event) {
          const item = atom.workspace.getCenter().getActivePaneItem();
          if (item instanceof TranquilBrowserModel) {
            TranquilBrowser.showFind();
        ...
        // atom-workspace scope so these always appear in the command palette, not
        // only while a browser view is focused. They target the center's active
        // browser so they work even when a dock (Find, tree-view) has focus.
        'tranquil-browser:toggle-url-bar': function () {
          const item = atom.workspace.getCenter().getActivePaneItem();
          if (!(item instanceof TranquilBrowserModel)) return;
          item.view?.toggleURLBar();
        },
        'tranquil-browser:go-back': function () {
          const item = atom.workspace.getCenter().getActivePaneItem();
          if (!(item instanceof TranquilBrowserModel)) return;
          item.view?.goBack();
        },
        'tranquil-browser:go-forward': function () {
          const item = atom.workspace.getCenter().getActivePaneItem();
          if (!(item instanceof TranquilBrowserModel)) return;
          item.view?.goForward();
        },
