...
const TranquilBrowser = {
  ...
  getTranquilBrowserUrl: function (url) {
    if (url.startsWith('tranquil-browser://history')) {
      ...
      // Build the exact file:// URL the fresh-open path uses (from __dirname, not
      // this.resources — `this` binding here isn't guaranteed) — a bare path or an
      // undefined base won't load in the webview and shows a white screen.
      return (url = (
        'file://' +
