...
const { windowHistoryKey, tabDisplayTitle, sanitizeFilename, resolveUserAgent } = require("./utils");
require("JSON2");
...
function handleGlobalGuestKey(evt) {
  ...
  // ⌥⌘W (ctrl-alt-w off mac) — close every tab in the center pane. A tabs
  // command, not a browser one, so it lives in tranquil-automations; here we
  // only forward it, matching evt.code (Option mangles evt.key to a dead char
  // on macOS). No menu accelerator carries it, so — unlike cmd-w below — the
  // guest path is the only delivery when a webview holds focus.
  if (
    evt.altKey &&
    evt.code === 'KeyW' &&
    (isMac ? evt.metaKey : evt.ctrlKey)
  ) {
    atom.commands.dispatch(workspaceEl, 'tranquil-automations:close-all-center-tabs');
    return true;
  }
  // ⇧⌘U (ctrl-shift-u off mac) — toggle the Automation Runs panel. An automations command, not a
  // browser one, so it lives in tranquil-automations; forwarded here because a focused guest
  // swallows it before Pulsar's keymap, and no menu accelerator carries it. evt.code, not
  // evt.key: Shift makes the letter case-dependent.
  if (
    evt.shiftKey &&
    evt.code === 'KeyU' &&
    (isMac ? evt.metaKey : evt.ctrlKey)
  ) {
    atom.commands.dispatch(workspaceEl, 'tranquil-automations:toggle-runs-panel');
    return true;
  }
...
const TranquilBrowser = {
  ...
  // page refreshes its bookmark), but if the existing file already holds this exact
  // URL the write is skipped and an "already saved" notice shows instead. Used by
  // the toolbar save button. opt+cmd+click on a page link uses saveNumberedLink
  // instead.
  ...
  activate: function (state) {
    ...
    atom.workspace.addOpener(
      (function (_this) {
        return function (filePath, options) {
          // `open-url-file-in-editor` passes { openAsText: true } to bypass this
          // opener so `open()` falls through to a plain text editor of the raw
          // .url file, rather than launching the browser tab.
          if (options && options.openAsText) return;
      ...
      (function (_this) {
        return function (url, opt) {
          try{
              ...
              // `false` (not bare `return`, i.e. undefined) — matching the
              // checkUrl() rejection below — so the opener still counts as
              // having claimed the URL. A plain `return` lets atom.workspace.open()
              // treat it as unclaimed and fall through to its default "unrecognized
              // URI -> open as a file path" behavior, creating a bogus TextEditor
              // named after the URL's last path segment (e.g. "blank") instead of
              // silently swallowing the duplicate event as intended.
              if (window.prevUrl === url && window.prevId && (Date.now()-window.prevId)< 50 ) return false;
              ...
              if (
                ...
                console.error('[tranquil-browser] Failed to open URL:', e)
              }
        };
      })(this)
    );
    ...
    // Browser tabs present a configurable User-Agent (tranquil.browserUserAgent;
    // default is a clean stock-Chrome string with the Electron/Tranquil tokens
    // stripped — see resolveUserAgent). New tabs pick it up via the <webview
    // useragent> attribute (tranquil-browser-view.js). Here we react to changes:
    // apply the new UA live to already-open tabs (takes effect on their next load),
    // and push it to the main process, which rewrites the User-Agent request header
    // on every guest request — covering subresources/WebSockets the attribute misses.
    // observe() fires immediately, so main gets the current UA at activation.
    const applyUserAgent = () => {
      const ua = resolveUserAgent();
      for (const item of atom.workspace.getPaneItems()) {
        if (!(item instanceof TranquilBrowserModel)) continue;
        const wv = item.view && item.view.htmlv && item.view.htmlv[0];
        try { if (wv) wv.setUserAgent(ua); } catch (_) { /* not attached yet */ }
      }
      ipcRenderer.send('set-user-agent', ua);
    };
    this.subscriptions.add(
      atom.config.observe('tranquil.browserUserAgent', applyUserAgent)
    );
    // A version check (tranquil.browserVersions) changes the clean default too,
    // which is what a blank override resolves to — re-apply on that as well.
    this.subscriptions.add(
      atom.config.observe('tranquil.browserVersions', applyUserAgent)
    );
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        // cmd-t: open a new browser tab in the active pane, same as
        // tranquil-browser:open-in-active-pane below — a file open in that
        // pane used to make this split off a new pane beside it instead
        // (getPosition() only kicks in when the active item is a text
        // editor), which read as cmd-t "sometimes" splitting depending on
        // what tab you started from.
    ...
    this.subscriptions.add(
      atom.commands.add('atom-workspace', {
        ...
        // Open the raw .url file as a text editor. The { openAsText: true } option
        // makes the .url opener above decline, so open() falls through to a plain
        // text editor instead of launching the browser tab.
        'tranquil-browser:open-url-file-in-editor': function (event) {
          const entry = event.target.closest('.entry');
          const filePath = entry && entry.getPath();
          if (filePath) atom.workspace.open(filePath, { openAsText: true });
        },
