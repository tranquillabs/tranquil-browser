...
class HTMLEditor extends Model {
  constructor({ tranquilBrowser, url, opt, filePath }) {
    ...
    // Restore the persisted page title so a reopened tab reports its <title>
    // via getTitle() immediately — before (or without) its webview reloading.
    // serialize() saves this as opt.title; without this line it was dropped, so
    // restored tabs fell back to the URL until visited.
    this.title = this.opt.title || null;
    // Same for the page favicon URL, so a restored tab shows its icon before the
    // webview re-fires page-favicon-updated.
    this.favIcon = this.opt.favIcon || null;
  ...
  // The browser opens in the center but may be dragged into the right or bottom
  // docks. The left dock is deliberately excluded: that's where the tree-view
  // lives, and keeping 'left' out of the allowed set is what makes the tabs
  // package refuse to move the browser there — so a drag onto the tree-view
  // falls through to saving the tab as a .url instead (tranquil-drag-drop).
  getAllowedLocations() {
    return this.allowedLocations || ['center', 'right', 'bottom'];
  }
  ...
  serialize() {
    return {
      data: {
        ...
        url: this.url,
        // Persist the source .url file so a restored tab still reports its path
        // (getPath). Without this, reloaded browser tabs deserialize with
        // filePath:null and the tree-view can no longer highlight their bookmark.
